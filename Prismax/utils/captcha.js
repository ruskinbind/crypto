...
const solveCapSolver = async (params) => {
  let retries = 5;
  try {
    // Step 1: Create a CAPTCHA task
    const taskResponse = await axios.post(
      ...
      {
        clientKey: config.API_KEY_CAPSOLVER,
        task: {
          type: "ReCaptchaV3TaskProxyLess",
          minScore: 0.9,
          isEnterprise: true,
...
async function solveCaptcha(
  ...
  } else if (config.TYPE_CAPTCHA === "capsolver") {
    return await solveCapSolver(params);
  } else if (config.TYPE_CAPTCHA === "omo") {
    return await solveOMO(params);
  }
